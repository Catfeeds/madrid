 ...
 * 中介电话库
 */
class ResoldStaffPhoneController extends AdminController
{
    //黑名单列表
    public function actionList($phone = "")
    {
        ...
        $dataProvider = ResoldStaffPhoneExt::model()->getList($criteria,40);
    ...
    public function actionAjaxDel()
    {
        if (Yii::app()->request->isPostRequest) {
            ...
            if ($ids && is_array($ids = explode(",", $ids)) && $ids = array_filter($ids)) {
                $success_flag = 0;
                foreach ($ids as $id) {
                    $black = ResoldStaffPhoneExt::model()->findByPk($id);
                    if ($black->delete()) {
                        $success_flag++;
                    }
                }

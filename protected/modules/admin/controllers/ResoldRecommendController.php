 ...
 * @date 2015-09-08
 */
class ResoldRecommendController extends AdminController
{
    /**
	 * 权限控制
	 */
    ...
    public function init()
    {
        ...
        if($this->recomCate===null)
            $this->recomCate = ResoldRecomCateExt::model()->findAll();
    }
    ...
    public function actionList($t='',$str='', $cid=0,$substation='')
    {
        ...
        $criteria = new CDbCriteria(array(
            ...
            'order' => 'sort desc,created desc,id desc',
        ));
        ...
        $dataProvider = ResoldRecomExt::model()->getList($criteria);
    ...
    {
        $model = $id ? ResoldRecomExt::model()->findByPk($id) : new ResoldRecomExt;
        ...
        {
            $model->attributes = Yii::app()->request->getPost('ResoldRecomExt', array());
            
            if($model->save())
                $this->setMessage('保存成功！', 'success', array($_SESSION['adminLastUrl']?$_SESSION['adminLastUrl']:'list','substations'=>$model->substation_id));
            else
                $this->setMessage('保存失败！', 'error');
        }
        if(!$id)
        {
            $model->type = Yii::app()->request->getQuery('type',0);
            $model->fid = Yii::app()->request->getQuery('fid',0);
        }
        $fang = [];
        if($model->type==1){
            $fang = ResoldEsfExt::model()->findByPk($model->fid); 
        }elseif($model->type==2) {
            $fang = ResoldZfExt::model()->findByPk($model->fid); 
        }
    ...
    {
        $model = new ResoldRecomExt;
        $model->attributes = Yii::app()->request->getPost('ResoldRecomExt',array());
        $htmlIds = array();//将显示的html ID属性存入此数组
        if($model->cate&&in_array($model->cate->pinyin,ResoldRecomExt::$substationCatePinyin)){
            $htmlIds[] = 'fenzhan';
        }
    ...
    {
        $model = $id ? ResoldRecomCateExt::model()->findByPk($id) : new ResoldRecomCateExt;
        if(Yii::app()->request->isPostRequest)
        {
            $model->attributes = Yii::app()->request->getPost('ResoldRecomCateExt', array());
            if($model->save())
                $this->setMessage('操作成功！','success', array('/admin/resoldRecommend/catelist'));
            else
    ...
    public function actionAjaxDelRecom()
    {
        ...
        if($ids!==false)
        {
            ...
            try
            {
                $data = ResoldRecomExt::model()->findAll($criteria);
                foreach($data as $v)
                {
                    $v->delete();
                }
    ...
    {
        $old = 0;
        $model = ResoldRecomExt::model()->findByPk($id);
        $old = $model->sort;
        $model->sort = $sort;
        if($model->save())

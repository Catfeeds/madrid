...
class PlotController extends AdminController
{
    ...
    public function actions()
    {
        ...
        return array(
            ...
            'evaluateConfig' => $alias.'evaluate.ConfigAction',
            ...
            'videoList' => $alias.'video.ListAction',
            'videoEdit' => $alias.'video.EditAction',
            'videoStatus' =>$alias.'video.AjaxStatusAction',
            'videoDelete' =>$alias.'video.DeleteAction',
            'videoQuery' =>$alias.'video.AjaxQueryAction'
        );
    }
    ...
    public function actionEdit($id=0)
    {
        ...
        $this->render('edit', array('parentArea' => $parentArea, 'childArea' => $childArea, 'plot' => $plot, 'maps' => $maps));
    }
     ...
     */
    public function actionList($type='', $value='', $status='', $new=1)
    {
        Yii::app()->user->setReturnUrl(Yii::app()->request->getUrl());
        $criteria = new CDbCriteria(array(
            'condition' => 'is_new=:new',
     ...
     * ajax设户型图封面
     */
    public function actionAjaxSetHouseTypeCover(){
        if (Yii::app()->request->isPostRequest) {
            $id = Yii::app()->request->getPost('id', 0);
    ...
    public function actionPricelist()
    {
        ...
        $this->layout = '/layouts/modal_base';
        $type = CHtml::listData(TagExt::model()->getTagByCate('jzlb')->normal()->findAll(), 'id', 'name');

        $data = PlotPriceExt::getListByHid($hid);
    ...
    {
        $this->layout = '/layouts/modal_base';
        $model = $id == 0 ? new PlotPriceExt : PlotPriceExt::model()->findByPk($id);
        $model->hid = $hid;
        $jglb = CHtml::listData(TagExt::model()->getTagByCate('jglb')->normal()->findAll(), 'id', 'name');

        if (Yii::app()->request->isPostRequest) {
            $model->attributes = Yii::app()->request->getPost('PlotPriceExt', array());
            if (Yii::app()->request->getPost('set_new', 0)>=1) {
                $model->setNewPrice();
            }
            if ($model->save()) {
                $this->setMessage('保存成功！', 'success',  array('plot/pricelist', 'hid'=>$hid));
            } else {
                $this->setMessage('保存失败！', 'error');
            }
        }
    ...
    public function actionPriceDel()
    {
        ...
        if ($id>0) {
            if (PlotPriceExt::model()->deleteAll(array('condition'=>'id = '.$id)) >0) {
     ...
     * param hid 楼盘id
     */
    public function actionAjaxGetHouseType()
    {
        $hid = Yii::app()->request->getParam('hid');
        $criteria = new CDbCriteria();
        $criteria->select = 'id,title';
        $criteria->condition = 'hid=:hid';
        $criteria->params = array(':hid' => $hid);
        $houseType = PlotHouseType::model()->findAll($criteria);
        foreach ($houseType as $v) {
            $data[] = array('id' => $v->id, 'name' => $v->title);
        }
        echo CJSON::encode(array('results' => $data));
    }

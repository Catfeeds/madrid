 ...
 * 小区二手房
 */
class PesfListAction extends CAction{
    public $plot;
    public function run()
	{
	    ...
	    //     		$farea = AreaExt::model()->findByPk($value->area);
					// $esf[] = ['id'=>$value->id,'title'=>$value->title,'size'=>$value->size,'image'=>$value->image,'bedroom'=>$value->bedroom,'livingroom'=>$value->livingroom,'bathroom'=>$value->bathroom,'area'=>isset($farea)?$farea->name:'无','price'=>$value->price,'created'=>date('Y.m.d',$value->created),'sale_time'=>date('Y.m.d',$value->sale_time),'hurry'=>date('Y.m.d',$value->hurry)];
	    //     	}
		}

		// $trans = ['esf'=>$esf,'totalNum'=>$count,'page'=>$page];
		// if($needFilter)
		// 	$trans = array_merge($trans,['filters'=>['chosen_pricetag'=>$pricetag,'chosen_sort'=>$sort,'chosen_minprice'=>$minprice,'chosen_area'=>$area,'chosen_street'=>$street,'chosen_source'=>$source,'chosen_bedroom'=>$bedroom,'chosen_ts'=>$ts,'chosen_maxprice'=>$maxprice,'chosen_size'=>$size,'chosen_school'=>$school]]);

		$this->controller->render('plot_esf',['esfs'=>$esfs->data,'pager'=>$pager,'plot'=>$plot,'school'=>$schoolkw,'esfNum'=>$count,'page'=>$page,'get'=>['bedroom'=>$bedroom,'sort'=>$sort,'kw'=>$kw]+$multiQuery+$tagDirect+$tagBetween]);
	}
}

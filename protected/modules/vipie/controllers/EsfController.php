<?php 
class EsfController extends VipieController{
	/**
     * [$staff 当前登录职员]
     * @var [type]
     */
     ...
     * @return [type]      [description]
     */
	public function actionPublish($id=0,$category=0)
	{
        $new_esf = $id == 0 ? new ResoldEsfExt : ResoldEsfExt::model()->findByPk($id);
        $category && $new_esf->category = $category;
		...
		if(Yii::app()->request->isPostRequest)
    	{
    		...
    		$values = Yii::app()->request->getPost('ResoldEsfExt', array());
  			...
  			//封面赋值
    		// $values['image'] = Yii::app()->request->getPost('image','');
            ...
            else
            {
                ...
                var_dump($errors[0][0]);exit;
            }
    	}
        switch ($new_esf->category) {
        	case '2':
        		$this->render('publish_sp',['esf'=>$new_esf]);
        		break;
        	case '3':
        		$this->render('publish_xzl',['esf'=>$new_esf]);
        		break;
            default:
	...
	public function actionSaleup()
	{
		...
		foreach ($transArr as $key => $value) {
			$$key = $transArr[$key] = Yii::app()->request->getQuery($key,'');
		}
		$transArr['action'] = '/vipie/esf/saleup';
        ...
        if($time_type && $start_time) {
        	str_replace('-', '/', $start_time);
        	$criteria->addCondition($time_type.'>=:startDate');
            $criteria->params[':startDate'] = TimeTools::getDayBeginTime(strtotime($start_time));
        }
        if($time_type && $end_time) {
        	str_replace('-', '/', $end_time);
        	$criteria->addCondition($time_type.'<=:startDate');
            $criteria->params[':startDate'] = TimeTools::getDayBeginTime(strtotime($end_time));
        }

        // var_dump($criteria);exit;
        $dataProvider = ResoldEsfExt::model()->undeleted()->getList($criteria,5);
        $plots = [];
    ...
    {
        $zf = ResoldZfExt::model()->findByPk(2);
        $this->render('test',['zf'=>$zf]);
    }
}
...
$this->pageTitle = $articledetail->title.'_'.$articledetail->cate->name.'_'.SM::urmConfig()->cityName().'房产资讯-'.SM::GlobalConfig()->siteName().'房产-'.SM::GlobalConfig()->siteName();
Yii::app()->clientScript->registerMetaTag($articledetail->cate->name.'，'.SM::GlobalConfig()->siteName().'房产，'.SM::urmConfig()->cityName().'房产网，'.SM::urmConfig()->cityName().'房产信息网','keywords');
...
$this->breadcrumbs = array(SM::urmConfig()->cityName().'房产资讯'=>$this->createUrl('index'),$articledetail->cate->name=>$this->createUrl('index',array('cateid'=>$articledetail->cate->id)),'资讯详情页');
...
<div class="wapper">
    <div class="news-l">
        ...
        <div class="blank10"></div>
        <p class="pl15 c-g9 mb10 fs14 mt30"><?php if(SM::articleConfig()->copyright()) {  echo SM::articleConfig()->copyright(); }else{ ?>以上信息仅供参考，最终以开发商公布为准。本稿件为<?php echo SM::GlobalConfig()->siteName(); ?>独家原创稿件，版权所有，引用或转载请注明出处。</p>
        <?php } ?>
        <div class="line"></div>

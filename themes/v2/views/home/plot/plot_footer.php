    ...
    <div class="houses-bottom">
        <div class="addition clearfix">
            ...
            <div class="item-block fs16 fl">
                ...
                <dl>
                    <dt  class="c-g6">近期优惠</dt>
                    <dd class="c-red">
                        <?php
                            if($discount = $this->plot->newDiscount) {
                                echo $discount->url ? CHtml::link(Tools::u8_title_substr($discount->title,20),$discount->url,['class'=>'c-sred','target'=>'_blank']) : Tools::u8_title_substr($discount->title,20);
                            } else {
                                echo '暂无';
                            }
                         ?>
        ...
        <ul class="tuij-box fs16">
            <li><strong>相似楼盘：</strong>
                <span>
                    <a target="_blank" href="<?php echo $this->createUrl('/home/plot/list',array('place'=>$this->plot->area))?>"><?php echo isset($this->siteArea[$this->plot->area])?$this->siteArea[$this->plot->area]:SM::urmConfig()->cityName()?>楼盘</a>
                </span>
                <span>
...
<div class="wapperout fixed_box">
    <div class="wapper guide-box">
        <ul class="fl">
            ...
            <li <?php if($this->getAction()->getId() == 'map'):?>class="current"<?php endif;?>><a href="<?php echo $this->createUrl('map',array('py'=>$this->plot->pinyin));?>">周边配套</a></li>
            ...
            <?php if($this->plot->old_id&&SM::pageUrlConfig()->homePlotIndexEsfListUrl()||$this->plot->data_conf['esfUrl']): ?>
            <li><a href="<?php echo isset($this->plot->data_conf['esfUrl'])&&$this->plot->data_conf['esfUrl']?$this->plot->data_conf['esfUrl']:str_replace('{id}',$this->plot->old_id,SM::pageUrlConfig()->homePlotIndexEsfListUrl()); ?>" target="_blank">二手房</a></li>
            <?php endif; ?>
            <?php if($this->plot->old_id&&SM::pageUrlConfig()->homePlotIndexEsfListUrl()||$this->plot->data_conf['zfUrl']): ?>
            <li><a href="<?php echo isset($this->plot->data_conf['zfUrl'])&&$this->plot->data_conf['zfUrl']?$this->plot->data_conf['zfUrl']:str_replace('{id}',$this->plot->old_id,SM::pageUrlConfig()->homePlotIndexZfListUrl()); ?>" target="_blank">租房</a></li>
            <?php endif; ?>
            <?php if(SM::urmConfig()->bbsTagPageUrl()&&$this->plot->tag_id): ?>
            <li><a href="<?php echo str_replace('{tagid}',$this->plot->tag_id, SM::urmConfig()->bbsTagPageUrl()); ?>" target="_blank">论坛</a></li>
            <?php endif;?>
        </ul>

<?php
 ...
 $this->pageTitle = ''.SM::urmConfig()->cityName().$areaone->name.'邻校房_'.SM::urmConfig()->cityName().$areaone->name.'在售邻校房-'.SM::GlobalConfig()->siteName().'房产-'.SM::GlobalConfig()->siteName();
 Yii::app()->clientScript->registerMetaTag(SM::urmConfig()->cityName().$areaone->name.'邻校房_'.SM::urmConfig()->cityName().$areaone->name.'在售邻校房','keywords');
 Yii::app()->clientScript->registerMetaTag(SM::GlobalConfig()->siteName().'为你提供'.SM::urmConfig()->cityName().$areaone->name.'邻校房信息，并且根据学校区域的不同划分出不同学校区域所属楼盘，为你购买'.SM::urmConfig()->cityName().$areaone->name.'邻校房提供最准确的邻校房楼盘信息。','description');
 $this->breadcrumbs = array(SM::urmConfig()->cityName().'邻校房'=>$this->createUrl('index'),$areaone->name.'邻校房');
?>

<?php
$this->pageTitle = '商家职员列表';
$this->breadcrumbs = array($this->pageTitle);
$staff = ResoldStaffExt::model()->findStaffByUid(Yii::app()->user->uid);
?>
<div class="table-toolbar">
    <div class="btn-group pull-left">
        <form class="form-inline">
            <div class="form-group">
                <?php echo CHtml::dropDownList('type',$type,array('name'=>'姓名','phone'=>'手机号'),array('class'=>'form-control','encode'=>false)); ?>
            </div>
            <div class="form-group">
                <?php echo CHtml::textField('value',$value,array('class'=>'form-control chose_text')) ?>
            </div>
            <div class="form-group">
                <?php echo CHtml::dropDownList('time_type',$time_type,array('created'=>'添加时间'),array('class'=>'form-control','encode'=>false)); ?>
            </div>
   ...
   <table class="table table-bordered table-striped table-condensed flip-content table-hover">
    ...
    <?php foreach($list as $k=>$v): ?>
        <tr>
            ...
            <td style="text-align:center;vertical-align: middle"><?php echo $v->uid; ?></td>
            <td class="text-center"><?=$v->name?></td>
            <td class="text-center"><?=$v->phone?></td>
            <td class="text-center"><?=$v->packages?$v['packages'][0]['name']:'无'?></td>            
            <td class="text-center"><a  data-url=<?=$this->createUrl('/admin/resoldStaff/packageEdit',['id'=>$v->id])?>><?php if($package = $v->packages){  $package = json_decode($package[0]['content'],true);echo $package['total_num'].'/'.$v->hurry_num.'/'.$package['appoint_num'];}else{echo SM::resoldConfig()->resoldExtraSaleNum->value.'/0/0';} echo time()-$v->id_expire>=0?' <b style="color:red">已到期</b>':'';  ?></a></td>
            ...
            <div class="btn-group">
                    ...
                    <?php foreach(Yii::app()->params['shopStatus'] as $key=>$v1){?>
                        <li>
                            <?=CHtml::ajaxLink($v1,$this->createUrl('/vip/shop/staffStatus',['kw'=>$v1=='正常'?'open':($v1=='禁用'?'close':'check'),'ids'=>$v->id]),['success'=>'function(){location.reload();}'])?>
                        </li>
                      <?php  }?>
                    </ul>
                </div>
            ...
            <?php if($staff->is_manager):?>
                    <a href="<?php echo $this->createUrl('/vip/shop/staffEdit',array('id'=>$v->id)); ?>" class="btn default btn-xs green"><i class="fa fa-edit"></i> 修改 </a>
            <?php elseif($staff->id==$v->id):?>
                    <a href="<?php echo $this->createUrl('/vip/shop/staffEdit',array('id'=>$v->id)); ?>" class="btn default btn-xs green"><i class="fa fa-edit"></i> 修改 </a>
            <?php endif;?>
            </td>
        </tr>
...
<div class="form-group">
    ...
    <?php echo CHtml::ajaxButton('启用所选', $this->createUrl('/vip/shop/staffStatus'), array('data'=>array('ids'=>'js:getChecked()','kw'=>'open'),'type'=>'post', 'success'=>'function(data){location.reload()}', 'beforeSend'=>'function(){if(!getChecked()){toastr.error("请至少选择一项！");return false;}}'), array('class'=>'btn btn-success btn-sm')); ?>
    <?php echo CHtml::ajaxButton('禁用所选', $this->createUrl('/vip/shop/staffStatus'), array('data'=>array('ids'=>'js:getChecked()','kw'=>'close'),'type'=>'post', 'success'=>'function(data){location.reload()}', 'beforeSend'=>'function(){if(!getChecked()){toastr.error("请至少选择一项！");return false;}}'), array('class'=>'btn btn-success btn-sm')); ?>
</div>
<?php endif;?>

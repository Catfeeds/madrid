...
?>
	...
	<div class="wapper">
		<div class="detail_l">
			...
			<div class="detail-top clearfix">
				...
				<div class="info">
					...
					<ul class="info-ul">
						<li class="long"><span>楼盘名称：</span><a href="" class="addr"><?=$plot->title?><em>（</em><?=$esf->areaInfo->name?> <?=$esf->streetInfo->name?><em>）</em><em>[</em>街景地图<em>]</em></a></li>
						...
						<li class="left"><span><em>等</em>级：</span><?=isset($esf->getEsfTag()['xzllevel'])?$esf->getEsfTag()['xzllevel']:''?></li>
						<li><span><em>装</em>修：</span><?=isset($esf->decoration)?TagExt::getNameByTag($esf->decoration):'未知'?></li>
						<li class="long"><span>配套设施：</span>
                            <?php $pt = isset($esf->getEsfTag('pt')['pt'])?$esf->getEsfTag('pt')['pt']:[];?>
                            <?php
                            $esfpt = '';
							if($pt){
			...
			<div class="map">
		        <div class="map-box">
		            <div id="ui-map-box" data-lat="<?=$esf->plot->map_lat?>" data-lng="<?=$esf->plot->map_lng?>"></div>
        		...
        		<p class="compare"><span class="left up">环比上月：<i class="iconfont icon-jiantou1"></i><em>0.88%</em></span><span class="down">同比去年：<i class="iconfont icon-jiantou1"></i><em>0.88%</em></span></p-->
    		</div>
		...
		<div class="detail_r">
			...
			<div class="publisher-box">
				...
				<div class="btns">
					...
					<a href="" target="_blank" class="enter-btn">进入店铺</a>
				</div>
				<div class="clearfix">
		...
		<?php endif;?>
			...
			<?php $this->widget('ViewRecordWidget',['url'=>'/resoldhome/esf/info','cssType'=>1,'type'=>1,'category'=>3])?>
			<div class="s-common-title"><span>本楼盘其他写字楼</span><a href="<?=$this->createUrl('list',['hid'=>$plot->id,'type'=>3])?>" target="_blank">更多&gt;</a></div>
			<ul class="right-list">
                <?php foreach ($plotOtherEsfs as $key => $value) {?>
    				<li><a href="<?=$this->createUrl('info',['id'=>$value->id])?>" target="_blank"><span class="name"><?=$value->bedroom?>室<?=$value->livingroom?>厅</span><span class="cate tac"><?=$value->size?>m²</span><span class="price"><?=$value->price?>万</span></a></li>
    			<?php }?>
			</ul>
			<div class="s-common-title"><span><?=$plot->areaInfo->name?>价格相近写字楼</span><a href="<?=$this->createUrl('list',['type'=>3])?>" target="_blank">更多&gt;</a></div>
			<?php $this->widget('SpXzlRightWidget',['type'=>2,'url'=>'/resoldhome/zf/info','model'=>$esf])?>

            <?php if($staff):?>
    			<?php $staffEsfs = ResoldEsfExt::model()->saling()->findAll(['condition'=>'uid=:uid and category=3','params'=>[':uid'=>$staff->uid],'order'=>'refresh_time desc,sale_time desc','limit'=>4]);
    			 if($staffEsfs): ?>
    			...
    			<?php foreach ($staffEsfs as $key => $value) {?>
    				<li><a href="<?=$this->createUrl('list',['hid'=>$value->plot->id,'type'=>3])?>" target="_blank"><span class="name"><?=$value->plot->title?> </span><span class="cate tac"><?=$value->bedroom?>室<?=$value->livingroom?>厅 </span><span class="price"><?=$value->price?>万</span></a></li>
    			<?php }?>
    			</ul>
    			<?php endif;?>
    		<?php endif;?>
	...
	<div class="wapper">
		...
		<div class="common-title"><span><?=$plot->title?>小区相册</span></div>
		<div class="fang-list long-list">
            <ul>
                <?php foreach ($plotAlbum as $key => $value) {?>
                <li>
                    <a href="<?=$this->createUrl('/resoldhome/plot/image',['py'=>$plot->pinyin,'pid'=>$value->id])?>">
                        <div class="pic">
            ...
            <div class="more-info">更多信息：<a href="<?=$this->createurl('/resoldhome/plot/index',['py'=>$plot->pinyin])?>" target="_blank"><?=$plot->title?></a><a href="<?=$this->createUrl('/home/plot/price',['py'=>$plot->pinyin])?>" target="_blank"><?=$plot->title?>房价走势</a><a href="<?=$this->createUrl('/resoldhome/plot/album',['py'=>$plot->pinyin])?>" target="_blank"><?=$plot->title?>相册</a><a href="http://www.hualongxiang.com/yezhu" target="_blank"><?=$plot->title?>业主论坛</a></div>
        </div>
    <?php endif;?>
        <div class="common-title"><span>您可能感兴趣的房源</span></div>
		<div class="fang-list long-list">
        <?php $this->widget('ViewRecordWidget',['url'=>'/resoldhome/esf/info','cssType'=>4,'type'=>1,'category'=>3,'limit'=>5])?>
	</div>
        <div class="blank10"></div>

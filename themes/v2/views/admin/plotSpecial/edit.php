...
<!--按照楼盘关联户型图数据-->
<div class="form-group">
    <label class="col-md-2 control-label">选择<?php echo $form->label($special, 'housetype_img')?></label>
    <div class="col-md-6 house-type">
        <?php echo $form->dropDownList($special,'htid',$htid , array('empty'=>'请选择户型','class'=>'form-control')) ?>
    </div>
</div>
...
<script>
    <?php Tools::startJs();?>
        ...
        $(".house").on('change',function(){
            var hid =$("#PlotSpecialExt_hid").val();
            $.ajax({
                url:'<?php echo $this->createUrl('plot/ajaxGetHouseType') ?>',
                data:{hid:hid},
                success:addSelect
            })
        })
        ...
        function addSelect(data){
            ...
            $('#PlotSpecialExt_htid').empty();
            $('#PlotSpecialExt_htid').append('<option value="">请选择户型</option>');
            returnData.results.forEach(function(e){
                var option = '<option value="'+ e.id+'">'+ e.name+'</option>';
                $('#PlotSpecialExt_htid').append(option);
            })
        }

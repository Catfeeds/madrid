...
<div class="table-toolbar">
    ...
    <div class="btn-group pull-right">
        <a href="<?php echo $this->createUrl('specialedit',array('referrer'=>Yii::app()->request->url)) ?>" class="btn green">
    ...
    <?php foreach($data as $v): ?>
        <tr>
            ...
            <td style="text-align:center;vertical-align: middle">
                ...
                <?php echo CHtml::ajaxLink(HousesSpecialExt::$status[$v->status],$this->createUrl('speciallist'), array('type'=>'post', 'data'=>array('id'=>$v->id, 'status'=>$v->status==1?0:1),'success'=>'function(data){location.reload()}'), array('class'=>'btn btn-sm '.HousesLibExt::$statusStyle[$v->status])); ?>
                <?php else:?>
                    <a class="btn btn-sm " href="javascript:;" ><?php echo HousesSpecialExt::$status[$v->status];?></a>
                <?php endif;?>
            </td>
            <td style="text-align:center;vertical-align: middle">
                <?php echo CHtml::ajaxLink(HousesSpecialExt::$is_recommend[$v->is_recommend],$this->createUrl('speciallist'), array('type'=>'post', 'data'=>array('id'=>$v->id, 'is_recommend'=>$v->is_recommend==1?0:1),'success'=>'function(data){location.reload()}'), array('class'=>'btn btn-sm '.HousesLibExt::$statusStyle[$v->is_recommend])); ?>
            </td>
			<td style="text-align:center;vertical-align: middle"><?php echo ($v->end_time > 0) ? date('Y-m-d',$v->end_time) : '暂无';?></td>
            <td style="text-align:center;vertical-align: middle">
                <a href="<?php echo $this->createUrl('/admin/house/specialedit',array('id'=>$v->id,'referrer'=>Yii::app()->request->url)) ?>" class="btn default btn-xs green"><i class="fa fa-edit"></i> 修改 </a>
                <?php
					echo CHtml::htmlButton('删除', array('data-toggle'=>'confirmation', 'class'=>'btn btn-xs yellow', 'data-title'=>'确认删除？', 'data-btn-ok-label'=>'确认', 'data-btn-cancel-label'=>'取消', 'data-popout'=>true,'ajax'=>array('url'=>$this->createUrl('specialdel'),'type'=>'post','success'=>'function(data){location.reload()}','data'=>array('id'=>$v->id))));
                 ?>
            </td>
        </tr>
    <?php endforeach; ?>    

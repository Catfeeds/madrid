    ...
    <?php foreach($list as $v): ?>
        <tr>
            ...
            <td class="text-center">
                <?php echo CHtml::ajaxLink(RecomExt::$status[$v->status]['name'],$this->createUrl('/admin/recommend/ajaxRecomStatus'), array('type'=>'post', 'data'=>array('ids'=>$v->id, 'status'=>$v->status),'success'=>'function(data){location.reload()}'), array('class'=>'btn btn-sm '.RecomExt::$status[$v->status]['class'])); ?>
            </td>
            <td class="text-center">
                <a href="<?php echo $this->createUrl('edit',array('id'=>$v['id'])) ?>" class="btn default btn-xs green"><i class="fa fa-edit"></i> 编辑 </a>
                <?php
                    echo CHtml::htmlButton('<i class="fa fa-times"></i> 删除', array('data-toggle'=>'confirmation', 'class'=>'btn btn-xs red', 'data-title'=>'确认删除？', 'data-btn-ok-label'=>'确认', 'data-btn-cancel-label'=>'取消', 'data-popout'=>true,'ajax'=>array('url'=>$this->createUrl('/admin/recommend/ajaxDelRecom'),'type'=>'post','success'=>'function(data){location.reload()}','data'=>array('ids'=>$v->id))));
                 ?>
            </td>
        </tr>
    <?php endforeach; ?>
...
<div class="form-group">
    ...
    <?php echo CHtml::ajaxButton('启用所选', $this->createUrl('/admin/recommend/ajaxRecomStatus'), array('data'=>array('ids'=>'js:getChecked()','status'=>0),'type'=>'post', 'success'=>'function(data){location.reload()}', 'beforeSend'=>'function(){if(!getChecked()){toastr.error("请至少选择一项！");return false;}}'), array('class'=>'btn btn-success btn-sm')); ?>
    <?php echo CHtml::ajaxButton('禁用所选', $this->createUrl('/admin/recommend/ajaxRecomStatus'), array('data'=>array('ids'=>'js:getChecked()','status'=>1),'type'=>'post', 'success'=>'function(data){location.reload()}', 'beforeSend'=>'function(){if(!getChecked()){toastr.error("请至少选择一项！");return false;}}'), array('class'=>'btn btn-success btn-sm')); ?>
    <?php echo  CHtml::ajaxButton('删除所选', $this->createUrl('/admin/recommend/ajaxDelRecom'), array('data'=>array('ids'=>'js:getChecked()'),'type'=>'post', 'success'=>'function(data){location.reload()}', 'beforeSend'=>'function(){if(!getChecked()){toastr.error("请至少选择一项！");return false;}}'), array('class'=>'btn btn-success btn-sm', 'data-toggle'=>'confirmation', 'data-title'=>'确认删除？', 'data-btn-ok-label'=>'确认', 'data-btn-cancel-label'=>'取消', 'data-popout'=>true)); ?>
</div>

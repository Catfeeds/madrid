    ...
    <div class="tab-pane active col-md-12" id="tab_1">
        ...
        <?php echo CHtml::htmlButton('添加流水', array('class'=>'btn btn-info', 'onclick'=>'js:$(this).hide().next().slideDown(function(){window.parent.resizeModal();});')); ?>
            <div style="display: none">
                <?php
                    echo $form->dropDownList($userLog, 'visit_status', UserExt::$visitStatus,array('class'=>'form-control input-inline','onchange'=>'js:if($(this).find("option:selected").text()=="已分配"){$(this).next().removeClass("hide");}else{$(this).next().addClass("hide")}'));

                ?>
                <span class="hide">
                    <?php
                        echo CHtml::dropDownList('staff_id', $user->staff_id, CHtml::listData(StaffExt::model()->normal()->staffIsWork()->findAll(),'id','username'), array('empty'=>array(0=>'--请选择买房顾问--'),'class'=>'form-control input-inline '));
                        if(SM::jikeConfig()->mode()){
                            echo CHtml::checkBox('rand', false, ['class'=>'form-control']),'随机平均分配';
                        }
                     ?>
                </span>
                <?php
        ...
        <?php $form = $this->beginWidget('HouseForm', array('action'=>$this->createUrl('edit'),'htmlOptions'=>array('class'=>'form-horizontal'))); ?>
            <div class="form-body">
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-5">
                        <?php echo $form->textField($user, 'wechat', array('class'=>'form-control')); ?>
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-md-2 control-label">微信验证是否通过</label>
                    <div class="col-md-5 radio-list">
                        <?php echo $form->radioButtonList($user, 'is_validate', UserExt::$is_validate, array('class'=>'form-control','separator'=>'','template'=>'<label>{input} {label}</label>')); ?>
                    </div>
                </div>
                <div class="form-group">

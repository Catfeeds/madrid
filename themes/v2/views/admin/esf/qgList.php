...
<div class="table-toolbar">
    <div class="btn-group pull-left">
        <form class="form-inline">
            ...
            <!--            时间脚本 end-->
             <div class="form-group">
                <?php echo CHtml::dropDownList('category',$category,['1'=>'住宅','2'=>'商铺','3'=>'写字楼'],array('class'=>'form-control chose_select','encode'=>false,'prompt'=>'--房源类型--')); ?>
            </div>
            <div class="form-group">
                <?php echo CHtml::dropDownList('status',$status,Yii::app()->params['qgStatus'],array('class'=>'form-control chose_select','encode'=>false,'prompt'=>'--审核状态--')); ?>
            </div>
    ...
    <div class="pull-right">
        ...
        <a href="<?php echo $this->createUrl('qgEdit') ?>" class="btn green">
            添加求购 <i class="fa fa-plus"></i>
        </a>
    </div>
   ...
   <table class="table table-bordered table-striped table-condensed flip-content table-hover">
    ...
    <?php foreach($list as $k=>$v): ?>
        <tr>
            ...
            <td class="text-center"><a target="_blank" href="<?=$this->createUrl('/resoldhome/qg/detail',['id'=>$v->id,'type'=>$v->category])?>"><?=$v->title?></a></td>
            <td class="text-center"><?=$v->username.' uid:'.$v->uid?></td>
            <td class="text-center"><?=$v->phone?></td>
            <td class="text-center"><?php if($plots = $v->getPlots()) foreach ($plots as $key => $value) {
                echo $value['plot_name'].' ';
            }?></td>
            <td class="text-center"><?=$v->bedroom.'/'.$v->livingroom.'/'.$v->bathroom?></td>
            <td class="text-center"><?=$v->price.'万元'?></td>
            ...
            <td class="text-center">
                <div class="btn-group">
                    ...
                    <?=Yii::app()->params['qgStatus'][$v->status]?> <i class="fa fa-angle-down"></i>
                    </button>
                    <ul class="dropdown-menu" role="menu">
                    <?php foreach(Yii::app()->params['qgStatus'] as $key=>$v1){?>
                        <li>
                            <?=CHtml::ajaxLink($v1,$this->createUrl('QgStatus',['kw'=>$key=='1'?'open':($key=='2'?'check':($key=='3'?'close':'weitongguo')),'ids'=>$v->id]),['success'=>'function(){location.reload();}'])?>
                        </li>
                      <?php  }?>
                    </ul>
                </div>
            ...
            <td style="text-align:center;vertical-align: middle">
                    <a href="<?php echo $this->createUrl('/admin/esf/qgEdit',array('id'=>$v->id)); ?>" class="btn default btn-xs green"><i class="fa fa-edit"></i> 修改 </a>
                    <?php
                        echo CHtml::htmlButton('删除', array('data-toggle'=>'confirmation', 'class'=>'btn btn-xs yellow', 'data-title'=>'确认删除？', 'data-btn-ok-label'=>'确认', 'data-btn-cancel-label'=>'取消', 'data-popout'=>true,'ajax'=>array('url'=>$this->createUrl('ajaxDelQg'),'type'=>'post','success'=>'function(d){if(d.code){location.reload()}else{toastr.error(d.msg)}}','data'=>array('id'=>$v->id))));
                ?>
            </td>
        </tr>
    <?php endforeach;?>
...
<div class="form-group">
    ...
    <?php echo CHtml::ajaxButton('启用所选', $this->createUrl('/admin/esf/QgStatus'), array('data'=>array('ids'=>'js:getChecked()','kw'=>'open'),'type'=>'post', 'success'=>'function(data){location.reload()}', 'beforeSend'=>'function(){if(!getChecked()){toastr.error("请至少选择一项！");return false;}}'), array('class'=>'btn btn-success btn-sm')); ?>
    <?php echo CHtml::ajaxButton('禁用所选', $this->createUrl('/admin/esf/QgStatus'), array('data'=>array('ids'=>'js:getChecked()','kw'=>'close'),'type'=>'post', 'success'=>'function(data){location.reload()}', 'beforeSend'=>'function(){if(!getChecked()){toastr.error("请至少选择一项！");return false;}}'), array('class'=>'btn btn-success btn-sm')); ?>
</div>

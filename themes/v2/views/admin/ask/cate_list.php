...
<?php Tools::startJs(); ?>
    $(function(){
        ...
        $('#do_sort').on('click',function(ev){
            ...
            $.getJSON('<?php echo $this->createUrl("cateEdieJson"); ?>',{data:dt},function(msg) {
                if(msg.code == 100) {
                    location.reload();
                }
            });
    ...
    function editAskCate(ts){
        ...
        edit_url = '<?php echo Yii::app()->createUrl("admin/ask/cateedit");?>'+'?id='+id;
    ...
    function del(id,obj){
            if(confirm("是否确定删除?")){
                // location.href="<?php echo $this->createUrl('catedel')?>?id="+id;
                var url = "<?php echo $this->createUrl('catedel')?>?id="+id;

    ...
    <div class="btn-group pull-left">
        <form class="form-inline">
            <div class="form-group">
                <?php echo CHtml::dropDownList('type',$type,array('title'=>'楼盘名称','id'=>'ID'),array('class'=>'form-control','encode'=>false)); ?>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <?php echo CHtml::dropDownList('new', $new, array(1=>'新盘',0=>'旧房'), array('class'=>'form-control','prompt'=>'--是否新房--')) ?>
            </div>
            <button type="submit" class="btn blue">搜索</button>
        </form>
    </div>
    <div class="pull-right">
        <a href="<?php echo $this->createUrl('/admin/plot/edit') ?>" class="btn green">
...
<table class="table table-bordered table-striped table-condensed flip-content table-hover">
    ...
    <tr>
        ...
        <th class="text-center" style="width: 150px">二手房排序</th>
        <th class="text-center" id="hname">楼盘名称</th>
        <th class="text-center">二手房均价</th>
        <th class="text-center">二手房总数</th>
        <th class="text-center">租房总数</th>
        <th class="text-center">价格动态</th>
    ...
    <?php foreach($list as $k=>$v): ?>
        <tr>
            ...
            <td class="warning sort_edit" data-id="<?php echo $v->id?>" style="text-align:center;vertical-align: middle"><?php echo $v->resoldSort ?></td>
            <?php
            $lab =  array(
            ...
            ?>
            <?php echo CHtml::openTag('td',$lab); ?><a href="<?php echo Yii::app()->createUrl('resoldhome/plot/index',array('py'=>$v->pinyin))?>" target="_blank" class="hname"><?php echo Tools::utf8substr($v->title,0,30); ?></a><?php echo CHtml::closeTag('td'); ?>
            <td style="text-align:center;vertical-align: middle"><?php echo $v->price ? $v->price.PlotPriceExt::$unit[$v->unit] : '暂无'; ?></td>
            <td style="text-align:center;vertical-align: middle"><?php $fcount = ResoldEsfExt::model()->undeleted()->count(['condition'=>'hid=:hid','params'=>[':hid'=>$v->id]]); echo $fcount?'<a href="'.$this->createUrl('/admin/esf/esfList',['hid'=>$v->id]).'">'.$fcount.'</a>':$fcount; ?></td>
            <td style="text-align:center;vertical-align: middle"><?php $fcount = ResoldZfExt::model()->undeleted()->count(['condition'=>'hid=:hid','params'=>[':hid'=>$v->id]]); echo $fcount?'<a href="'.$this->createUrl('/admin/zf/zfList',['hid'=>$v->id]).'">'.$fcount.'</a>':$fcount; ?></td>
            <td style="text-align:center;vertical-align: middle">
                <div class="btn-group btn-group-xs btn-group-solid">
                    <a class="label label-danger" onclick="do_admin($(this))" data-url="<?php echo $this->createUrl('/admin/resoldPlot/priceList',array('hid'=> $v->id)) ?>">二手房价格动态</a>
                </div>
...
<?php Tools::startJs(); ?>
    ...
    function set_sort(_this, id, sort){
        $.getJSON("<?php echo $this->createUrl('/admin/resoldPlot/changeResoldSort');?>",{hid:id,sort:sort},function(dt){

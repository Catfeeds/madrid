...
<div class="table-toolbar">
    <div class="btn-group pull-left">
        <form class="form-inline">
            <div class="form-group">
                <?php echo CHtml::dropDownList('type',$type,array('name'=>'套餐名称'),array('class'=>'form-control','encode'=>false)); ?>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <?php echo CHtml::dropDownList('checkStatus',$checkStatus,Yii::app()->params['shopStatus'],array('class'=>'form-control chose_select','encode'=>false,'prompt'=>'--状态--')); ?>
            </div>
    ...
    <div class="pull-right">
        <a href="<?php echo $this->createUrl('packageEdit') ?>" class="btn green">
            添加套餐 <i class="fa fa-plus"></i>
        </a>
    </div>
</div>
   <table class="table table-bordered table-striped table-condensed flip-content table-hover">
    ...
    <tr>
        ...
        <th class="text-center">上架数/预约数</th>
    ...
    <?php foreach($list as $k=>$v): ?>
        <tr>
            ...
            <td class="text-center"><?php if($v->content) {$package = json_decode($v['content'],true); echo $package['total_num'].'/'.$package['appoint_num'];}else{echo '无';}?></td>
            ...
            <div class="btn-group">
                    ...
                    <?php foreach(Yii::app()->params['shopStatus'] as $key=>$v1){?>
                        <li>
                            <?=CHtml::ajaxLink($v1,$this->createUrl('packageStatus',['kw'=>$v1=='正常'?'open':($v1=='禁用'?'close':'check'),'ids'=>$v->id]),['success'=>'function(){location.reload();}'])?>
                        </li>
                      <?php  }?>
                    </ul>
                </div>
            ...
            <td style="text-align:center;vertical-align: middle">
                    <a href="<?php echo $this->createUrl('/admin/resoldTariffPackage/packageEdit',array('id'=>$v->id)); ?>" class="btn default btn-xs green"><i class="fa fa-edit"></i> 修改 </a>
                    <?php
                        echo CHtml::htmlButton('删除', array('data-toggle'=>'confirmation', 'class'=>'btn btn-xs yellow', 'data-title'=>'确认删除？', 'data-btn-ok-label'=>'确认', 'data-btn-cancel-label'=>'取消', 'data-popout'=>true,'ajax'=>array('url'=>$this->createUrl('AjaxDelPackage'),'type'=>'post','success'=>'function(d){if(d.code){location.reload()}else{toastr.error(d.msg)}}','data'=>array('id'=>$v->id))));
                ?>
            </td>
        </tr>
    <?php endforeach;?>
...
<div class="form-group">
    ...
    <?php echo CHtml::ajaxButton('启用所选', $this->createUrl('/admin/resoldTariffPackage/packageStatus'), array('data'=>array('ids'=>'js:getChecked()','kw'=>'open'),'type'=>'post', 'success'=>'function(data){location.reload()}', 'beforeSend'=>'function(){if(!getChecked()){toastr.error("请至少选择一项！");return false;}}'), array('class'=>'btn btn-success btn-sm')); ?>
    <?php echo CHtml::ajaxButton('禁用所选', $this->createUrl('/admin/resoldTariffPackage/packageStatus'), array('data'=>array('ids'=>'js:getChecked()','kw'=>'close'),'type'=>'post', 'success'=>'function(data){location.reload()}', 'beforeSend'=>'function(){if(!getChecked()){toastr.error("请至少选择一项！");return false;}}'), array('class'=>'btn btn-success btn-sm')); ?>
</div>
...
<?php Tools::startJs(); ?>
    ...
    function set_sort(_this, id, sort){
            $.getJSON('<?php echo $this->createUrl('/admin/resoldTariffPackage/ajaxPackageSort')?>',{id:id,sort:sort},function(dt){
                location.reload();
            });
        }
    function do_sort(ts){

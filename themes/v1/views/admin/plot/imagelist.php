...
<div class="table-toolbar">
    <form method="get" action="<?php $this->createUrl('/admin/plot/imagelist')?>">
        ...
        <div class="btn-group" id="selroom" data-hx="<?php echo TagExt::getHxId();?>">
            <?php echo CHtml::dropDownList('hx',$hx,PlotExt::$room,array('class'=>'form-control','style'=>'display:block','encode'=>false)); ?>
        </div>
    ...
    <?php foreach($data as $v):?>
        <tr>
            ...
            <td style="text-align:center;vertical-align: middle">
                <?php echo $ctype[$v->type];?>
            </td>
            <?php if($ctype[$v->type] == '户型图'){
                $lab =  array(
                    'data-original-title'=>'图片详情',
                    'data-content'=> '房型 :'.$v->room.' 室<br>面积 : '.$v->size.' ㎡',
                    'data-html'=>'true',
            ...
            ?>
            <td style="text-align:center;vertical-align: middle" <?php echo $str;?>>
...
<script>
    <?php Tools::startJs(); ?>
        var hxtype = $("#selroom").data("hx");
        var type = $("#type").val();
        if(type == hxtype){
            $("#hx").css("display","block");
            $("#hx").attr("disabled",false);
        }else{
            $("#hx").css("display","none");
            $("#hx").attr("disabled","disabled");
        }
        $("#type").click(function(){
            var type = $("#type").val();
            if(type == hxtype){

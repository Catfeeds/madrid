...
<div class="table-toolbar">
    ...
    <div class="btn-group pull-right">
        <a href="<?php echo $this->createAbsoluteUrl('/admin/plotSpecial/edit',array('referrer'=>Yii::app()->request->url)) ?>" class="btn green">
    ...
    <?php foreach($data as $v): ?>
        <tr>
            ...
            <td style="text-align:center;vertical-align: middle">
                ...
                <?php echo CHtml::ajaxLink(PlotSpecialExt::$status[$v->status],$this->createAbsoluteUrl('list'), array('type'=>'post', 'data'=>array('id'=>$v->id, 'status'=>$v->status==1?0:1),'success'=>'function(data){location.reload()}'), array('class'=>'btn btn-sm '.PlotExt::$statusStyle[$v->status])); ?>
                <?php else:?>
                    <a class="btn btn-sm " href="javascript:;" ><?php echo PlotSpecialExt::$status[$v->status];?></a>
                <?php endif;?>
            </td>
            <td style="text-align:center;vertical-align: middle">
                <?php echo CHtml::ajaxLink(($v->recommend == 0) ? '否' : '是',$this->createAbsoluteUrl('list'), array('type'=>'post', 'data'=>array('id'=>$v->id, 'recommend'=>($v->recommend!=0) ? 0 : time()),'success'=>'function(data){location.reload()}'), array('class'=>'btn btn-sm '.PlotExt::$statusStyle[$v->recommend == 0 ? 0 : 1 ])); ?>
            </td>
            ...
            <td style="text-align:center;vertical-align: middle">
                ...
                <a href="<?php echo $this->createAbsoluteUrl('/admin/plotSpecial/edit',array('id'=>$v->id,'referrer'=>Yii::app()->request->url)) ?>" class="btn default btn-xs green"><i class="fa fa-edit"></i> 修改 </a>
                <?php

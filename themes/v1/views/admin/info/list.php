...
<!--            <td class="warning sort_edit" data-id="--><?php //echo $v->id?><!--" style="text-align:center;vertical-align: middle">--><?php //echo $v->sort ?><!--</td>-->
            ...
            <td>
                <a href="<?php echo $this->createAbsoluteUrl('edit',array('id'=>$v->id,'referrer'=>Yii::app()->request->url)) ?>" class="btn default btn-xs green"><i class="fa fa-edit"></i> 编辑 </a>
...
<div class="form-group">
    ...
    <?php echo CHtml::ajaxButton('启用所选', $this->createAbsoluteUrl('ajaxChangeStatus'), array('data'=>array('id'=>'js:getChecked()'),'type'=>'post', 'success'=>'function(data){location.reload()}', 'beforeSend'=>'function(){if(!getChecked()){toastr.error("请至少选择一项！");return false;}}'), array('class'=>'btn btn-success btn-sm')); ?>
    <?php echo !isset($_GET['status']) ? CHtml::ajaxButton('禁用所选', $this->createAbsoluteUrl('ajaxChangeStatus'), array('data'=>array('id'=>'js:getChecked()'),'type'=>'post', 'success'=>'function(data){location.reload()}', 'beforeSend'=>'function(){if(!getChecked()){toastr.error("请至少选择一项！");return false;}}'), array('class'=>'btn btn-success btn-sm')) : ''; ?>
    <?php echo !isset($_GET['status']) ? CHtml::ajaxButton('删除所选', $this->createAbsoluteUrl('ajaxArticleDel'), array('data'=>array('id'=>'js:getChecked()'),'type'=>'post', 'success'=>'function(data){location.reload()}', 'beforeSend'=>'function(){if(!getChecked()){toastr.error("请至少选择一项！");return false;}}'), array('class'=>'btn btn-success btn-sm', 'data-toggle'=>'confirmation', 'data-title'=>'确认删除？', 'data-btn-ok-label'=>'确认', 'data-btn-cancel-label'=>'取消', 'data-popout'=>true)) : ''; ?>
    <?php echo !isset($_GET['status']) ? CHtml::ajaxButton('批量关联楼盘', $this->createAbsoluteUrl('ajaxPlotRelated'), array('data'=>array('id'=>'js:getChecked()'),'type'=>'post', 'success'=>'function(data){location.reload()}',
    'beforeSend'=>'function(){
            if(!getChecked())
                {
...
<?php $this->widget('AdminLinkPager', array('pages'=>$pager)) ?>
    <script language="JavaScript">
        function set_sort(_this, id, sort){
            $.getJSON('<?php echo $this->createAbsoluteUrl('EditArticleSort')?>',{id:id,sort:sort},function(dt){

...
<div class="table-toolbar">
    ...
    <div class="btn-group pull-right">
        <a href="<?php echo $this->createAbsoluteUrl('/admin/school/edit',array('referrer'=>Yii::app()->request->url))?>" class="btn green">
...
<table class="table table-bordered table-striped table-condensed flip-content">
    ...
    <?php foreach($data as $v): ?>
        <tr>
            ...
            <td style="text-align:center;vertical-align: middle">
                <?php echo CHtml::ajaxLink(SchoolExt::$status[$v->status],$this->createAbsoluteUrl('ajaxStatus'), array('type'=>'post', 'data'=>array('id'=>$v->id, 'status'=>$v->status==1?0:1),'success'=>'function(data){location.reload()}'), array('class'=>'btn btn-sm '.PlotExt::$statusStyle[$v->status])); ?>
            </td>
            <td style="text-align:center;vertical-align: middle">
                <?php echo CHtml::ajaxLink(SchoolExt::$recommend[$v->recommend != 0 ? 1 : 0],$this->createAbsoluteUrl('ajaxRecom'), array('type'=>'post', 'data'=>array('id'=>$v->id, 'recommend'=>$v->recommend != 0 ? 0 : time()),'success'=>'function(data){location.reload()}'), array('class'=>'btn btn-sm '.PlotExt::$statusStyle[$v->recommend != 0 ? 1 : 0])); ?>
            </td>
            ...
            <td style="text-align:center;vertical-align: middle">
                ...
                <?php
                    echo CHtml::htmlButton('删除', array('data-toggle'=>'confirmation', 'class'=>'btn btn-xs red', 'data-title'=>'确认删除？', 'data-btn-ok-label'=>'确认', 'data-btn-cancel-label'=>'取消', 'data-popout'=>true,'ajax'=>array('url'=>$this->createAbsoluteUrl('del'),'type'=>'post','success'=>'function(data){location.reload()}','data'=>array('id'=>$v->id))));
                 ?>
            </td>
        </tr>
    <?php endforeach; ?>

    ...
    <tbody>
        ...
        <tr>
            ...
            <td style="text-align:center;vertical-align: middle">
                <?php echo CHtml::ajaxLink(SchoolExt::$status[$v->status],$this->createAbsoluteUrl('ajaxSchoolRelStatus'), array('type'=>'post', 'data'=>array('id'=>$v->id, 'status'=>$v->status==1?0:1),'success'=>'function(data){location.reload()}'), array('class'=>'btn btn-sm '.PlotExt::$statusStyle[$v->status])); ?>
            </td>
            <td style="text-align:center;vertical-align: middle">
                <a href="<?php echo $this->createAbsoluteUrl('/admin/school/areaedit',array('id'=>$v->id))?>" class="btn default btn-xs green"><i class="fa fa-edit"></i>修改</a>
                <?php
...
<?php $this->widget('AdminLinkPager', array('pages'=>$pager)) ?>
    <script type="text/javascript">
        function set_sort(_this, id, sort){
            $.getJSON('<?php echo $this->createAbsoluteUrl('/admin/school/EditSchoolAreaSort')?>',{id:id,sort:sort},function(dt){

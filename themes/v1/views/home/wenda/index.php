...
$this->breadcrumbs = array($this->siteConfig['cityName'].'问答');
if(isset($cate) && !empty($cate)){
    foreach($cate as $v){
        if(isset($v['childs']) && !empty($v['childs'])){
            foreach($v['childs'] as $n){
                if($cid == $n['id']){
                    $this->breadcrumbs = array($this->siteConfig['cityName'].'问答'=>$this->createUrl('index'),$n['name']);
                }
            }
        }
    }
    ...
    <div class="main-block wenda-index">
        ...
        <div class="main-right k-fl">
            ...
            <div class="sub-block clearfix">
                <div class="sub-left">
                    <h3 class="wenda-aside-title"><?php if(isset($cid) && !empty($cid)){ $acate = AskCateExt::model()->find(array('condition'=>'id = :id','params'=>array(':id'=>$cid))); echo $acate->name; }else{ ?>最新问题<?php } ?></h3>
                    <ul>
